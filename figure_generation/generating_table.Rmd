...
NLR_popgen <- read_tsv("//wsl.localhost//Ubuntu//home//chandlersutherland//scratch//egglib_results.tsv") %>% 
  filter(domain=='cds')%>%
  mutate(Pi=Pi_raw/dna_aln) %>%
  subset(select=c('Gene', 'D','Pi'))
  
NLR_popgen 

#rest of the genome from vcf as described in whole_genome_popgen.ipynb
whole_genome <- read_csv("//wsl.localhost//Ubuntu//home//chandlersutherland//scratch//cds_egglib_wholegenome2.csv") %>% 
  subset(select=c('gene', 'Pi_by_dna', 'D')) %>%
  rename('Pi_by_dna'='Pi', 'gene'='Gene') %>% distinct()
