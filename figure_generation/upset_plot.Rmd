...
Fisher test of independence between HV status and cluster status 
```{r}
cont_table <- alt %>% 
...
```
HV status and cluster status are independent 
...
```{r}
p_val <- pivot_alt %>% compare_means(meth_percentage~HV, ., group.by='subgroup', paired=F, method='wilcox.test', p.adjust.method = 'BH') %>%
  arrange(subgroup, p.adj) %>% 
  unique() %>%
  mutate(x=seq(from=1, to=group_n)) %>% 
  mutate(y=90) %>% 
